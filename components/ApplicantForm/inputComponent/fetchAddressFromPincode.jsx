const fetchAddressFromPincode = async (formData, handleChange, addressType, pincode, setPincodeError, controller) => {
  ...
  try {
    const response = await fetch(`https://api.postalpincode.in/pincode/${pincode}`, { signal });
      ...
      if (data && data.length > 0 && data[0].PostOffice.length > 0) {
        ...
        for (let i = 0; i < addressTypeKeys.length; i++) {
          if (i === addressTypeKeys.length - 1) {
            ...
            currentLevel[addressTypeKeys[i]] = { ...currentLevel[addressTypeKeys[i]] };
