...
const PersonalInfo = ({ formData, handleChange, presentPincodeError, setPresentPincodeError, permanentPincodeError, setPermanentPincodeError }) => {
  const copyAddress = (e) => {
  ...
  useEffect(() => {
    ...
    if (presentPincode.length === 6) {
      fetchAddressFromPincode(formData, handleChange, 'personal_info.present_address', presentPincode, setPresentPincodeError, controller);
    }
    return () => {
      controller.abort();
    };
  ...
  useEffect(() => {
    ...
    if (permanentPincode.length === 6) {
      fetchAddressFromPincode(formData, handleChange, 'personal_info.permanent_address', permanentPincode, setPermanentPincodeError, controller);
    }
    return () => {
      controller.abort();
    };
